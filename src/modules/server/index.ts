...
import {
  errorServerCreate,
  errorServerApplyMiddleware,
  errorServerStartListenning,
} from "../error";
...
class Server {
  ...
  constructor(config: ServerConfig) {
    const serverConfig = ramda.mergeDeepRight(
    ...
    return new Promise<boolean>((resolve, reject) => {
      try {
        this.server.listen(this.serverConfig.port, () => {
          Logger.success(
            `Server is listenning on port ${this.serverConfig.port} `
          );
          return resolve(true);
        });
      } catch (err) {
