...
export class MongooseRepository {
    ...
    constructor(collectionName: string) {
        this.collection = mongoose.model(collectionName, Schemas[collectionName]);
    }
    ...
    async delete(id: string | mongoose.Schema.Types.ObjectId, isRealDelete: boolean = false): Promise<boolean> {
        if (!isRealDelete) {
            let result = await this.collection.update({_id: MongooseFunction.toObjectId(id)}, {deletedAt: new Date()}).exec();
            return result && result.ok > 0;
        }
        await this.collection.remove({_id: MongooseFunction.toObjectId(id)}).exec();
        return true;
    }
}
